...
【核心原则】生成方向性的任务列表，每个任务是一个探索方向，不是具体指令。（用户已在上一步确认过项目方向，直接生成任务即可，不需要再判断是否清晰）
- 任务是"做什么方向"，不是"具体怎么做"
- 让用户自己决定具体内容和方式
- 任务名称简短（3-8个字）
- 参考方向（不是必须）：知识查找、知识回顾、动手制作、整理输出等

请生成任务列表。要求：
1) 分为2-3个阶段
2) 每个阶段包含2-3个方向性任务
3) 每个任务有：标题（简短的方向名）、预计时长（纯数字，单位分钟，15-60之间）、简短说明（一句话点明这个方向的意义，不写具体做法）
4) 不要使用 Markdown
